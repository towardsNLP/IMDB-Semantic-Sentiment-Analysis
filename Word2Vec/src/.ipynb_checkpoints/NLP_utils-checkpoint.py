...
def train_logistic_regressor(train_data_bow_matrix, 
    ...
    evaluate_model(y_true = train_data_label, y_pred = train_data_predict_label)
...
def w2v_trainer(doc_tokens: List[str],
               ...
               vector_size: int = 300,
                ...
                min_count: int = 2):
    """ 
    Going through a list of lists, where each list within the main list contains a set of tokens from a doc, this function train a Word2Vec model,
...
def calculate_topn_similarity_score(keyed_vectors, 
    ...
    then returns the similarity scores. 
    To calculate the similarity score it calculates the similarity of every word in the target_tokens set 
