...
def bow_sentiment_analysis_model_training(train_data_bow_matrix, 
    ...
    model_evaluation_metrics(y_true = train_data_label, y_pred = train_data_predict_label)
...
def w2v_trainer(doc_tokens: List[str]):
    ...
    """
    # TODO: move the variables to the function level
    # TODO: update gensim package
...
def overall_similarity_score(keyed_vectors,
...
def topn_similarity_score(keyed_vectors, 
                          target_tokens: List[str], 
                          doc_tokens: List[str],
                          topn: int = 10) -> float:
